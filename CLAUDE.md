...
3. **禁止 sed 操作 HTML/JS**：用 Python 脚本精确操作。修改 JS 函数时，禁止用 `str.replace()` 局部替换——必须用正则找到函数起止边界，整体重写整个函数块。修改后必须 `grep` 验证新代码存在且旧代码已移除。
4. **小步提交**：每完成一个功能点，立即 `git add` + `git commit`
5. **先方案后代码**：复杂改动先输出修改方案（改哪些文件、改哪几行），用户确认后再写
6. **提交前验证**：`node -c` 检查语法 + `curl` 测试端点。禁止提交未验证的代码。
7. **不懂就问**：遇到不确定的逻辑，必须询问用户，禁止猜测
8. **15分钟止损**：同一问题尝试两次还不行，立即切换方案。特别是 bash heredoc 转义失败时，立刻改用本地 Python 文件 + scp 方式。
9. **🔴 禁止 heredoc 嵌入 Python**：这是最常见的事故源。Python 脚本必须写成本地 `.py` 文件，通过 `scp` 上传服务器执行。绝对禁止在 bash heredoc 中嵌入 Python 代码。

   **为什么会炸**：Python 代码里的 `${}`、`()`、`"`、`'`、`--amber`、反引号等字符会被 bash 在传给 Python 之前先解释一遍。即使加了 `<< 'PYEOF'` 禁止变量展开，嵌套引号和特殊字符仍然会随机触发语法错误。这是 bash 和 Python 两种语法体系的根本冲突，无法靠"更小心的转义"解决。

   **正确做法**：
   ```bash
   # ❌ 禁止：bash heredoc 嵌 Python
   ssh luoyu "python3 << 'PYEOF'
   ... 复杂 Python 代码 ...
   PYEOF"
   
   # ✅ 正确：本地写 .py → scp 上传 → ssh 执行
   # 1. Write tool 创建 C:\Users\qixuan\fix_xxx.py
   # 2. scp 上传
   # 3. ssh luoyu "python3 /tmp/fix_xxx.py"
   # 4. 验证结果
   ```
   
10. **路由冲突检查**：修改 API 端点前，必须 `grep -n` 搜索所有 `routes/*.js` 文件确认同一路径没有被多个文件处理。
